...
export const PlaylistCard = ({ id, name, count, img }: any) => {
  const { shuffleTracks, setRefresh, createPlaylist, addTracksToPlaylist, user, setShuffleLoad } =
  ...
  return (
    <Card className="p-2 mb-[0.75rem]">
      <Flex align={'center'} justify={'between'} >
        <Flex className='overflow-hidden' align={'center'} gap={'6'}>
          <Avatar color="ruby" fallback={name.charAt(0)} src={img} />
          <Text className='' weight={'light'} size={'5'}>
            {truncateName(name)}
          </Text>
        </Flex>
        <Flex  align={'center'} gap={'9'}  justify={'between'}>
          <Text className='text-nowrap break-keep h-6 truncate pl-2 sm:hidden' weight={'light'} color="gray" size={'5'}>
            {count} songs
          </Text>
